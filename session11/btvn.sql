...
-- cau 1
delimiter //
create procedure getPosts(in p_user_id int)
BEGIN
...
-- cau 2
delimiter //
create procedure postLikes( in p_post_id int, out total_likes int)
BEGIN
    select count(*) into total_likes from likes where post_id = p_post_id;
END //
delimiter ;
SET @likes_count = 0;
CALL postLikes(101, @likes_count);
SELECT @likes_count;
drop procedure postLikes;

-- cau 3
delimiter $$

create procedure calBonusPoints(in p_user_id int, inout p_bonus_points int)
BEGIN
    if (select count(*) from posts where user_id = p_user_id) >= 20 then
        set p_bonus_points = p_bonus_points + 100;
    elseif (select count(*) from posts where user_id = p_user_id) >= 10 then 
...
SET @bonus_points = 0;
CALL calBonusPoints(1, @bonus_points);
SELECT @bonus_points;
DROP PROCEDURE IF EXISTS calBonusPoints;

delimiter //
create procedure postValidation( in p_user_id int, in p_content text, out result_message varchar(255))
BEGIN
    if char_length(p_content) < 5 then set result_message = 'Nội dung quá ngắn';
    else insert into posts(user_id, content, created_at) values (p_user_id, p_content, now());
...
delimiter ;
SET @m = '';
CALL postValidation(1, 'Ni', @m);
SELECT @m;
SET @n= '';
CALL postValidation(1, 'Neck Hurt', @n);
SELECT @n;