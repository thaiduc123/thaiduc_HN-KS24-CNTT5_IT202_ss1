...
select room_type, price_per_day from rooms order by price_per_day desc;
select * from rooms where (price_per_day>1000000);
select * from bookings where(check_in >= '2024-01-01');
select room_type, count(*) from rooms group by room_type;

-- phan 2
select b.booking_id, g.guest_name, r.room_type, b.check_in
from bookings b
...
select b.booking_id, g.guest_name, r.room_type, r.price_per_day, (b.check_out - b.check_in) * r.price_per_day as 'so tien'
from bookings b
join guests g on b.guest_id = g.guest_id join rooms r on b.room_id = r.room_id;

select r.room_type, sum((b.check_out - b.check_in) * r.price_per_day) as 'tong doanh thu'
from  bookings b join rooms r on b.room_id = r.room_id group by r.room_type;

select g.guest_id, g.guest_name, g.phone, count(b.booking_id)
from guests g
join bookings b on g.guest_id = b.guest_id group by g.guest_id, g.guest_name
having count(b.booking_id) >= 2;

select r.room_type, count(b.booking_id) as 'so lan dat'
from bookings b
join rooms r on b.room_id = r.room_id group by r.room_type order by count(b.booking_id) desc limit 1;
